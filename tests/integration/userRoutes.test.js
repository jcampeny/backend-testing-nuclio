...
describe('User Routes Integration Tests', () => {
    ...
    describe('POST /api/users/upload', () => {
        test('Debería subir una imagen de perfil con datos válidos', async () => {
            ...
            expect(res.statusCode).toBe(200);
            expect(res.body).toHaveProperty('message', 'Imagen actualizada');

            // Verificar que la imagen se ha actualizado en la base de datos
            const User = mongoose.model('User');
            const user = await User.findOne({email: 'pedro@example.com'});
            expect(user).not.toBeNull();
            expect(user.profileImage).toBe('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...');
        });
